...
//--------栏目管理-----------
	...
	//添加栏目操作（需要支持图片上传，需重写添加方法）
	public function tfAddSite(){
		$info=$this->upload('site',1);
		//有图片上传处理
		if($info){
			$_POST['litpic']=$info[0]['savename'];
	...
	//更新栏目/文章公共方法操作（需要支持图片上传，需重写添加方法）
	public function tfUpdasite(){
		$info=$this->upload('site',1);
		//有图片上传处理
		if($info){
			$_POST['litpic']=$info[0]['savename'];
		...
		if($add->create()){
			...
			if($ret1){
				...
				if($art->create()){
					...
					$ret2 = $art->add();
					if($ret2){
						$this->Record('文章添加成功');//后台操作
						$this->success("添加成功",'__APP__/TIFAWEB_DSWJCMS/Site/articleList');
					}else{
						$this->error( "添加失败art");
					}
				}else{
