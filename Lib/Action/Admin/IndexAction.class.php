...
//--------系统设置-----------
    ...
    public function addsy(){
            ...
            if($last){
				...
				F('systems',NULL);
				$this->success('添加成功', '__APP__/TIFAWEB_DSWJCMS/Index/system.html');
            }else{
    ...
    public function delesy(){
			...
			F('systems',NULL);
			$this->success('删除成功', '__APP__/TIFAWEB_DSWJCMS/Index/system.html');
    }
    ...
    public function savesys(){
			...
			if($create=$system->create()){	
				...
				$this->success('更新成功', '__APP__/TIFAWEB_DSWJCMS/Index/system.html');
			}else{
				$this->error($system->getError());
			}            
    }
		...
		if($system->create()){	
			...
			if($info){
				foreach($info as $i=>$in){
					$datas['id']			= $_POST['id'][$in['key']];
					$datas['value']		= $in['savename'];
					$system->save($datas);
				}
			}
			F('systems',NULL);
    ...
    public function email_send(){
			...
			if($system->create()){
				 ...
				 if($result){
					 ...
					 $this->success('更新成功', '__APP__/TIFAWEB_DSWJCMS/Index/email.html');
...
//--------管理操作记录-----------
     public function operation(){
       if($this->_get('title')){
		...
		if($this->_get('time')){
			$_GET['time']=str_replace('+', ' ',$this->_get('time'));
			$time=explode(" 至 ",$this->_get('time'));
...
//--------会员操作记录-----------
	...
	//--------后台模板-----------
    public function admcolour(){
		$directory = F('admdirectory'); 	//模板
		$dirname = F('admdirname');  // 默认模板
		if(!$directory){
			$directory=$this->templateData('./Tpl/Admin/template/');
			F('admdirectory',$directory);
			$directory=F('admdirectory');	
		}
		if(!$dirname){
			$dir="Default";
			F('admdirname',$dir);
			$dirname=F('admdirname');
		}
		$this->assign('num',$directory['num']);
		unset($directory['num']);
		$this->assign('dirname',$dirname);
	...
	//后台模板设为默认
	public function admsetDefault(){
		if($this->_post('dir')){
			F('admdirname',NULL);
			F('admdirname',$this->_post('dir'));
			$dirname=F('admdirname');
			$directory = F('admdirectory');
			$num=$directory['num'];
			unset($directory['num']);
	...
	//后台模板刷新
	public function admcolourRefresh(){
		if($this->_post('limit')==1){
			$dirname = F('admdirname');
			F('admdirectory',NULL);
			$directory=$this->templateData('./Tpl/Admin/template/');
	...
	//头像上传
	public function adminlogoupload(){
		$info=$this->upload('adminportrait',1);
		//有图片上传处理
		if($info){
			$data['portrait']=$info[0]['savename'];
			M('admin')->where(array('id'=>$this->_session('admin_uid')))->save($data);
			session('admin_portrait',$data['portrait']);
			session('admin_portraittype',null);
			
		}else if($this->_post('portrait')){
